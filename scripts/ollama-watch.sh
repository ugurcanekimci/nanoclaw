...
while true; do
  ...
  for status_file in data/ipc/*/ollama_status.json; do
    ...
    # Sanitize DETAIL: replace double-quotes with single-quotes to prevent
    # shell injection via osascript -e "..." string interpolation.
    SAFE_DETAIL="${DETAIL//\"/\'}"

    case "$STATUS" in
      generating)
        osascript -e "display notification \"${SAFE_DETAIL}\" with title \"NanoClaw → Ollama\" sound name \"Submarine\"" 2>/dev/null
        echo "$(date +%H:%M:%S) 🔄 $DETAIL"
        ;;
      done)
        osascript -e "display notification \"${SAFE_DETAIL}\" with title \"NanoClaw ← Ollama ✓\" sound name \"Glass\"" 2>/dev/null
        echo "$(date +%H:%M:%S) ✅ $DETAIL"
        ;;
      listing)
