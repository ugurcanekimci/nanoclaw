...
export const transcriptMcpServer = createSdkMcpServer({
  ...
  tools: [
    ...
    tool(
      ...
      async (args) => {
        const entry = await ingestYouTubeVideo(
          args.url,
          args.language,
          args.tags,
        );
    ...
    tool(
      ...
      'Fetch transcripts for multiple YouTube URLs. Stores all in Obsidian vault.',
      {
        urls: z
          ...
          .min(1)
          .max(50)
          .describe('YouTube URLs or video IDs'),
    ...
    tool(
      ...
      'Fetch a tweet or thread from X/Twitter. Uses Nitter (free) → Crawl4AI → Apify cascade. Stores in Obsidian vault.',
      {
        url: z
          .string()
          .describe('Tweet URL (x.com or twitter.com) or tweet ID'),
      },
      async (args) => {
        ...
        const output = [
          ...
          truncateToTokenBudget(
            tweet.content,
            2000,
            'use kb_search for full content',
          ),
        ].join('\n');
    ...
    tool(
      ...
      async (args) => {
        ...
        const lines = tweets.map(
          (t) => `- [[x-posts/${t.tweetId}]] ${t.content.slice(0, 80)}...`,
        );
    ...
    tool(
      ...
      'Search Obsidian vault across all content types. Supports compound queries: type:youtube channel:"Name" tag:topic keyword',
      {
        query: z
          .string()
          .describe(
            'Search query — supports type:, tag:, channel:, author: prefixes',
          ),
      },
      async (args) => {
        ...
        if (results.length === 0) {
          return {
            content: [
              { type: 'text' as const, text: `No results for "${args.query}"` },
            ],
          };
        }
        ...
        const text = results
          .map((r) => {
            const subdir =
              r.entry.type === 'youtube-transcript'
                ? 'youtube'
                : r.entry.type === 'x-post'
                  ? 'x-posts'
                  : 'research';
    ...
    tool(
      ...
      async (args) => {
        ...
        if (entries.length === 0) {
          return {
            content: [
              { type: 'text' as const, text: 'Knowledge base is empty.' },
            ],
          };
        }
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `${entries.length} recent entries:\n\n${text}`,
            },
          ],
        };
      },
      ...
      async (args) => {
        ...
        if (entries.length === 0) {
          return {
            content: [
              {
                type: 'text' as const,
                text: `No entries tagged "${args.tag}"`,
              },
            ],
          };
        }
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `${entries.length} entries tagged #${args.tag}:\n\n${text}`,
            },
          ],
        };
      },
    ...
    tool(
      ...
      'Write a research note to the Obsidian vault. Use for saving research findings, analysis, or any content that should persist in the knowledge base.',
      {
        slug: z
          .string()
          .describe(
            "URL-safe filename without extension (e.g. 'vitest-mocking-patterns')",
          ),
        ...
        sources: z
          .array(z.string())
          .default([])
          .describe('Source URLs or references'),
      },
      async (args) => {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Saved: [[research/${args.slug}]] at ${filePath}`,
            },
          ],
        };
      },
    ...
    tool(
      ...
      {
        subdir: z
          .enum(['youtube', 'x-posts', 'research'])
          .describe('Vault subdirectory'),
      ...
      async (args) => {
        ...
        if (!content) {
          return {
            content: [
              {
                type: 'text' as const,
                text: `Note not found: ${args.subdir}/${args.filename}`,
              },
            ],
            isError: true,
          };
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: truncateToTokenBudget(content, config.maxToolResultTokens),
            },
          ],
        };
      },
      ...
      async (args) => {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: truncateToTokenBudget(output, config.maxToolResultTokens),
            },
          ],
        };
      },
    ...
    tool(
      ...
      'Return the LangFuse dashboard URL for a given agent session. Useful for debugging agent behavior and inspecting token usage.',
      {
        session_id: z
          .string()
          .describe(
            "Agent session ID (typically the group folder, e.g. 'slack_swarm-coder')",
          ),
      },
      async (args) => {
        ...
        if (!publicKey || !process.env.LANGFUSE_SECRET_KEY) {
          return {
            content: [
              {
                type: 'text' as const,
                text: 'LangFuse is not configured (LANGFUSE_PUBLIC_KEY / LANGFUSE_SECRET_KEY not set).',
              },
            ],
          };
        }
...
function formatEntryLine(e: IndexEntry): string {
  ...
  const prefix =
    e.type === 'youtube-transcript' ? 'YT' : e.type === 'x-post' ? 'X' : 'R';
  const meta =
    e.type === 'youtube-transcript'
